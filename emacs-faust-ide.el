...
(defun emacs-faust-ide-mode-build (&optional build-all)
  "Build the executable generated by this Faust code file using the `emacs-faust-ide-build-target' executable"
  (interactive)
  ...
  (with-current-buffer (get-buffer-create "Faust output")
    ...
    (if build-all
        (progn (setq-local file-to-build "*.dsp")
               (message "Building just your"))
      (progn (message "Building ALL")
